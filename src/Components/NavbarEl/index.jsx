...
import { Menu as MenuIcon, Close as CloseIcon, ExpandMore, EditNote, Explore } from '@mui/icons-material';
...
const NavbarEl3 = () => {
  ...
  return (
    <>
      <nav
        className={`fixed w-full z-[1000] transition-all duration-300 ${isSticky ? 'bg-white shadow-md' : 'bg-white'}`}
      >
          ...
          <div className="hidden sm:flex gap-6 items-center">
            {menus.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`text-sm font-medium ${pathname === item.href ? 'text-blue-600' : 'text-gray-700'} hover:text-blue-600`}
              >
                {item.label}
              </Link>
            ))}
            ...
            >
              <button className="text-sm text-gray-700 hover:text-blue-600 font-medium flex items-center gap-1">
                All Services <ExpandMore fontSize="small" />
              </button>
              {isServicesOpen && (
                <div className="absolute top-full left-0 w-[680px] bg-white shadow-lg rounded-lg p-6 grid grid-cols-2 gap-6 z-50">
                  <div>
                    <h1
                    ...
                    >
                      ...
                      {hoverRevenue && (
                        <span className="absolute top-8 left-0 w-72 text-xs bg-white p-2 shadow rounded text-gray-500">
                          All knowledge is derived from real-time experience and executed market data, ensuring fewer mistakes.
                        </span>
                      )}
                    </h1>
                    <div className="flex flex-col mt-2">
                      <Link href="/lead-generation-process-setup" className="text-sm text-gray-700 hover:text-blue-600">Lead Generation Process Setup</Link>
                      <Link href="/strategic-account-management" className="text-sm text-gray-700 hover:text-blue-600">Strategic Account Management</Link>
                      <Link href="/market-analysis" className="text-sm text-gray-700 hover:text-blue-600">Market Analysis</Link>
                      <Link href="/marketing-communication" className="text-sm text-gray-700 hover:text-blue-600">Marketing Communication Strategy</Link>
                      <Link href="/community-building" className="text-sm text-gray-700 hover:text-blue-600">Community Building</Link>
                    </div>
                  </div>
                  <div>
                    <h1 className="text-base font-semibold mb-2">Sales and Marketing Team Management Solution</h1>
                    <Link
